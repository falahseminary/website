...
function start_nginx {
  echo "### Starting nginx ..."
  docker-compose -f docker-compose.production.yml up --force-recreate -d nginx_service
  echo
}
...
function reload_nginx {
  ...
  if $(docker-compose -f docker-compose.production.yml exec nginx_service nginx -s reload); then
    echo "### Reloaded."
  else
    YELO='\033[1;33m'
    CYAN='\033[0;36m'
